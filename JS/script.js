...
// Update the totals with validation and amount in words
function updateTotals() {
    let grossEarnings = 0;
    let totalDeductions = 0;
    ...
    document.getElementById('total-deductions-final').textContent = `₹ ${totalDeductions}`;
    document.getElementById('net-payable-display').textContent = `₹ ${netPayable}`;
    document.getElementById('net-payable-words').textContent =
        `Amount in words: Indian Rupee ${numberToWords(netPayable)} Only`;
}
